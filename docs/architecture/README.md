...
本文档介绍 Paseo Kanban 的模块分层、拖拽交互、排序规则与并发控制。
...
Paseo Kanban 运行在 Paseo 宿主环境中。核心设计包括：

1. **零本地原生编译依赖（Zero Native Modules Overhead）**：
   - 使用 React Native 内置的 `Animated` 和布局能力实现占位槽与落位动画，无须额外的原生动效依赖。

2. **单一真实源与无冗余次序建模（Implicit Array Ordering）**：
   - `board.tasks` 的数组顺序表达任务次序，持久化数据不需要单独的 `order` 或 `rank` 字段。

3. **事务基准与乐观并发（Transaction Baseline Locking & Optimistic Concurrency）**：
   - 用户触发拖拽抓起或打开编辑弹窗的瞬间，系统即刻捕获当前时点的看板快照（`baseBoard`）、版本号（`baseRevision`）与项目筛选状态，锁定为不可变的事务基准；
   - 保存时传入打开会话时的 `revision`；若版本冲突，则保留草稿并提示用户刷新。

4. **响应式解耦与请求乱序防护（Order-Controlled Reactive Pipeline）**：
   - 项目订阅使用 `fetchProjectsWithOrderControl` 的递增请求 ID 处理乱序响应；项目列表请求失败时保留最近一次成功结果，看板数据操作继续运行。

---

## 2. 系统分层与模块拓扑

系统按宿主、客户端、事务和领域模型四层组织：
...
`KanbanDragController` 与 `KanbanCard` 分别处理鼠标、触屏及手柄输入：
...
[用户按下卡片]
   ...
   触屏端 (Touch):
     ...
     ├─ 220ms 内保持静止 ──> [触发长按抓起] ──> 抓取卡片、激活互斥锁
     └─ < 220ms 且无位移松手 ──> [识别为轻点] ──> 打开任务详情编辑弹窗
```

- **手势互斥锁（Lifecycle Mutex）**：控制器在拖拽期间设置 `dragLock`，避免释放手势时触发卡片点击；落位后经过 60ms 缓冲再解锁。
...
2. **三轨并行动画**：
   - `Animated.spring`：将悬浮卡片移动到目标占位槽（摩擦力 9，张力 90）；
   - `Animated.timing`：将抓起时的微倾斜角度（rotate: 1 -> 0）与放大缩放（scale: 1.03 -> 1.0）复位；
3. **落位提交**：动画完成后携带 `operationId` 调用 `reportDropComplete`；取消或过期的操作不再提交。

---

## 4. 排序算法与领域规则 (ADR-0001 & ADR-0002)

任务排序由纯函数 `reorderTask` 处理，按当前筛选和目标泳道计算新数组。
